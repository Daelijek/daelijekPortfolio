...
export default function ParticlesCanvas() {
  ...
  useEffect(() => {
    ...
    const render = (currentTime = performance.now()) => {
      ...
      // On touch / mobile devices, provide autonomous gentle orbital drifting
      if (isMobileOrTouch) {
        mouseRef.current.targetX = width * 0.5 + Math.sin(time * 0.4) * (width * 0.35);
        mouseRef.current.targetY = height * 0.5 + Math.cos(time * 0.3) * (height * 0.3);
      }
